...
const permiso = require('../googleAds/permiso');
...
router.get('/oauth/callback', async (req, res) => {
  ...
  const revision = permiso.revisarEstado(req.query.state);
  if (!revision.ok) {
    const mensaje = revision.motivo === 'expirado'
      ? 'Los enlaces de permiso duran 15 minutos. Pide uno nuevo e intentalo otra vez.'
      : 'Este enlace no lo genero el servidor, o ya se uso. Pide uno nuevo.';
    return res.status(400).send(pagina(revision.motivo === 'expirado' ? 'El enlace expiro' : 'Enlace no valido', mensaje, 'error'));
  }
  if (!req.query.code) {
  ...
  try {
    await permiso.canjearCodigo(req.query.code);
    ...
    console.error('[google-ads] No se pudo guardar el permiso:', err.message);
    res.status(502).send(pagina('Google no devolvio el permiso', 'Revisa que el ID y el secreto del cliente OAuth sean los correctos y que la URI de redireccion registrada sea exactamente la de este servidor.', 'error'));
  }
});
...
router.get('/oauth/inicio', ruta(async (req, res) => {
  const enlace = permiso.generarEnlace();
  res.json({
    ...enlace,
...
// Si el permiso ya esta guardado, y de donde sale.
router.get('/oauth/estado', ruta(async (req, res) => {
  res.json({
    permisoGuardado: permiso.permisoGuardado(),
    origen: GOOGLE_ADS.refreshToken ? 'variable de entorno' : (permiso.permisoGuardado() ? 'base de datos' : null),
    redirectUriRegistrada: permiso.REDIRECT_URI,
  });
}));

router.delete('/oauth', ruta(async (req, res) => {
  permiso.olvidarPermiso();
